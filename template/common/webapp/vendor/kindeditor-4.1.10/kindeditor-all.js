...
_extend(KUploadButton, {
	init : function(options) {
		...
		self.div = div;
		self.button = button;
        self.fileUpload = K('input[type="file"].ke-upload-file')
        self.uploadUrl = url;
	...
	submit : function() {
		var self = this;
        if(self.fileUpload[0].files.length == 0){
            alert('请选择要上传的图片')
            return
        }
        ...
        formData.append('imgFile', self.fileUpload[0].files[0])
        xhr.send(formData)
		return self;
	},
	remove : function() {
